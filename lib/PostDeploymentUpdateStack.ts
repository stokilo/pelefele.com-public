...
export class PostDeploymentUpdateStack extends Stack {
  constructor (scope: App, id: string, props: AppStackProps) {
    ...
    const sWait = new sfn.Wait(this, constructId('post-deployment-update-stack-sfn-wait', props), {
      time: sfn.WaitTime.duration(cdk.Duration.seconds(10))
    })

    const hostedZoneId = props.allStagesSecrets!.secretValueFromJson(`${props.stageUpperCase}_HOSTED_ZONE_ID`).toString()
    const handlerFunction = new Function(this, constructId('post-deployment-update-lambda', props), {
      enableLiveDev: false,
    ...
    const postDeploymentUpdateLambda = new tasks.LambdaInvoke(this, constructId('post-deployment-update-lambda-invoke', props), {
      lambdaFunction: handlerFunction,
      outputPath: '$.Payload'
    })
    const sFailed = new sfn.Fail(this, constructId('post-deployment-update-stack-sfn-fail', props))
    const sSuccess = new sfn.Succeed(this, constructId('post-deployment-update-stack-sfn-succeed', props))

    props.postDeploymentUpdateStackStateMachine = new sfn.StateMachine(this, constructId('post-deployment-update-state-machine', props), {
      timeout: Duration.minutes(10),
      definition: sWait
        .next(postDeploymentUpdateLambda)
        .next(
          new sfn.Choice(this, 'Is post deployment update finalized?')
            .when(sfn.Condition.stringEquals('$.status', StepFunctionResultSuccess), sSuccess)
            .otherwise(sFailed)
        )
    })

    props.postDeploymentUpdateStackStateMachine.applyRemovalPolicy(RemovalPolicy.DESTROY)
  }
}
