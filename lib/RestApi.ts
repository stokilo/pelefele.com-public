...
import { AuthorizationType, CognitoUserPoolsAuthorizer, Cors, MethodLoggingLevel } from 'aws-cdk-lib/aws-apigateway'
...
export default class RestApi extends sst.Stack {
  constructor (scope: sst.App, id: string, props: AppStackProps) {
    ...
    props.restApi = new sst.ApiGatewayV1Api(this, constructId('rest-api', props), {
      defaultAuthorizationType: AuthorizationType.COGNITO,
      defaultAuthorizer: new CognitoUserPoolsAuthorizer(
        this, constructId('cognito-authorizer', props), {
      ...
      restApi: {
        defaultCorsPreflightOptions: {
          ...
          allowOrigins: ['http://localhost:3000', 'https://dev.pelefele.com', 'https://pelefele.com'],
      ...
      defaultFunctionProps: {
        environment: {
          ...
          LISTING_SQS_DLQ_URL: props.listingStreamDQL?.sqsQueue.queueUrl!,
          ...
          IMG_UPLOAD_BUCKET_NAME: props.isDev ? APP_BUCKET_NAMES.DEV_UPLOAD_BUCKET : props.isLocal ? APP_BUCKET_NAMES.LOCAL_UPLOAD_BUCKET : APP_BUCKET_NAMES.PROD_UPLOAD_BUCKET,
          IMG_BUCKET_NAME: props.isDev ? APP_BUCKET_NAMES.DEV_ASSETS_BUCKET : props.isLocal ? APP_BUCKET_NAMES.LOCAL_ASSETS_BUCKET : APP_BUCKET_NAMES.PROD_ASSETS_BUCKET
        }
      },
      customDomain: {
    ...
    props.restApi.restApi.addUsagePlan(constructId('rest-api-usage-plan', props), {
      apiStages: [{
        api: props.restApi.restApi,
        stage: props.restApi.restApi.deploymentStage
      }],
      throttle: {
    ...
    props.restApi.attachPermissions([props.bucketConfig.applicationConfigBucket,
