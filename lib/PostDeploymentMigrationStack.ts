...
export class PostDeploymentMigrationStack extends Stack {
  constructor(scope: App, id: string, props: AppStackProps) {
    ...
    const sWait = new sfn.Wait(
      this,
      constructId('post-deployment-migration-sfn-wait', props),
      {
    ...
    const handlerFunction = new Function(
      this,
      constructId('post-deployment-migration-lambda', props),
      {
        enableLiveDev: false,
    ...
    const postDeploymentMigrationLambda = new tasks.LambdaInvoke(
      this,
      constructId('post-deployment-migration-lambda-invoke', props),
      {
    ...
    const sFailed = new sfn.Fail(
      this,
      constructId('post-deployment-migration-sfn-fail', props)
    )
    const sSuccess = new sfn.Succeed(
      this,
      constructId('post-deployment-migration-sfn-succeed', props)
    )

    props.postDeploymentMigrationStateMachine = new sfn.StateMachine(
      this,
      constructId('post-deployment-migration-state-machine', props),
      {
    ...
    props.postDeploymentMigrationStateMachine.applyRemovalPolicy(
