...
import { DynamoDBStreamsClient, GetRecordsCommand, GetShardIteratorCommand } from '@aws-sdk/client-dynamodb-streams'
import { unmarshall } from '@aws-sdk/util-dynamodb'
import {
  SystemInformationEntity
} from '../model/admin/SystemInformationEntity'
...
export class SystemAdminService {
  ...
  async migrateNext (dryRun: boolean = false): Promise<boolean> {
    ...
    logger.info(`Received call to migrateNext, dryRun? ${dryRun} nextMigrationNumber ${nextMigrationNumber}`)
    logger.info(systemInformation)

    if (this.allMigrations.length < nextMigrationNumber) {
      logger.info(`Skipping migration, no new migration with higher version than ${nextMigrationNumber}`)
      return false
    }
    ...
    systemInformation.lastMigrationNumber = systemInformation.lastMigrationNumber + 1
    if (!dryRun) {
      await systemInformationEntity.save(systemInformation)
    }

    const hasMoreMigrations = systemInformation.lastMigrationNumber < this.allMigrations.length
  ...
  async sqldlq (): Promise<boolean> {
    ...
    const dynamoStreamClient = new DynamoDBStreamsClient({ region: process.env.REGION })
    if (response.Messages) {
      for (let i = 0; i < response.Messages.length; i++) {
        const msg = response.Messages![i]
        const body = JSON.parse(msg.Body as string).DDBStreamBatchInfo

        const iterator = await dynamoStreamClient.send(new GetShardIteratorCommand({
          StreamArn: body.streamArn,
          ShardId: body.shardId,
          ShardIteratorType: 'AT_SEQUENCE_NUMBER',
          SequenceNumber: body.startSequenceNumber
        }))
        const records = await dynamoStreamClient.send(new GetRecordsCommand({
