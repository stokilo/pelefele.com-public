...
import {
  Listing, LISTING_SORTS,
  LISTING_TARGETS,
  LISTING_TYPES,
  ListingSelect, newTestListing, newTestListingResult
...
export class ListingSearchStore extends VuexModule {
  ...
  get isSearchDisabled () {
    return !process.env.isDevMode && (!this.listingSearch.locationPk.length ||
  ...
  get listingsGroupedBy3 (): Array<Array<Listing>> {
    return this.listingsFound.reduce<Array<Array<Listing>>>(
      function (previousValue: Array<Array<Listing>>, currentValue: Listing, currentIndex: number, array: Array<Listing>) {
  ...
  @mutation mutateListingData (mutatedData: ListingsSearchResults) {
    this.listingsFound = mutatedData.listings ? mutatedData.listings as Array<Listing> : []
    this.pagingTotalCount = mutatedData.totalCount > 10000 ? 1000 : mutatedData.totalCount
  ...
  async onSearch () {
    if (process.env.isMockMode) {
      return this.mutateListingData(newTestListingResult([newTestListing()], this.pagingCurrentPage))
    }

    const res = await this.apiCall.$get<ListingsSearchResults, typeof ListingsSearchResultsSchema>(
